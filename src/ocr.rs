...
async fn get_ocr_engine() -> Result<&'static OcrEngine> {
    ...
    let engine = task::spawn_blocking(move || {
        suppress_output(|| {
            OcrEngine::new(&det_path_str, &rec_path_str, &charset_path_str, None)
    ...
    OCR_ENGINE
        .set(engine)
        .map_err(|_| anyhow::anyhow!("Failed to set OCR engine"))?;
...
pub async fn recognize_image_from_path(path: &str) -> Result<OcrResult> {
    ...
    // Load image
    let image = image::open(path).map_err(|e| anyhow::anyhow!("Failed to load image: {}", e))?;
